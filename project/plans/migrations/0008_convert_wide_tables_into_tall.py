...
def convert_wide_to_tall(apps, schema_editor):
    ...
    for plan in IncomePlan.objects.all():
        for idx, month_name in enumerate(MONTH_NAMES, start=1):
            ...
            if price is not None:
                new_incomes.append(IncomePlan(
                    year=plan.year, month=idx, price=price,
                    income_type=plan.income_type, journal=plan.journal
                ))
    ...
    for plan in ExpensePlan.objects.all():
        for idx, month_name in enumerate(MONTH_NAMES, start=1):
            price = getattr(plan, month_name)
            if price is not None:
                new_expenses.append(ExpensePlan(
                    year=plan.year, month=idx, price=price,
                    expense_type=plan.expense_type, journal=plan.journal
                ))
    ...
    for plan in SavingPlan.objects.all():
        for idx, month_name in enumerate(MONTH_NAMES, start=1):
            price = getattr(plan, month_name)
            if price is not None:
                new_savings.append(SavingPlan(
                    year=plan.year, month=idx, price=price,
                    saving_type=plan.saving_type, journal=plan.journal
                ))
    ...
    for plan in NecessaryPlan.objects.all():
        for idx, month_name in enumerate(MONTH_NAMES, start=1):
            price = getattr(plan, month_name)
            if price is not None:
                new_necessary.append(NecessaryPlan(
                    year=plan.year, month=idx, price=price,
                    title=plan.title, expense_type=plan.expense_type, journal=plan.journal
                ))
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(convert_wide_to_tall, reverse_code=migrations.RunPython.noop),
    ]