...
def test_object_exists_204(client_logged):
...
def test_object_no_exists_204(client_logged):
...
def test_string_instead_int(client_logged):
...
def test_already_reseted(client_logged):
...
def test_reset_no_object(client_logged):
    url = reverse('bookkeeping:accounts_worth_reset', kwargs={'pk': 1})
    client_logged.get(url)
...
def test_reset_other_journal_account(client_logged, main_user, second_user):
...
def test_reset_no_worth_record(client_logged):
    a = AccountFactory()
